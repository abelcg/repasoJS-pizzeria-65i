...
const pizzasMedium = () => {
  return pizzaMenuCopy.filter((pizza) => pizza.prices.medium >= 850);
};
...
const searchByName = (name) => {
  return pizzaMenuCopy.filter((pizza) => pizza.name.includes(name));
  //return pizzaMenuCopy.filter(pizza => pizza.name.indexOf(name) !== 1)
};
...
orderPizzasByPrice();
console.log(pizzaMenuCopy);

// 4- A partir de un conjunto de IDs devuelva una lista de pizzas con dichos IDs

const getPizzasByIds = (arraysIDs) =>
  pizzaMenuCopy.filter((pizza) => arraysIDs.includes(pizza.id));
//const getPizzasByIds = (arraysIDs)=> pizzaMenuCopy.filter(pizza => arraysIDs.indexOf(pizza.id) !== -1)

console.log(getPizzasByIds([5]));
...
let uniqueIngredients = totalIngredients.filter(
  (item, index) => totalIngredients.indexOf(item) === index
);
...
const totlaIngredientsOrder = () => {
  ...
  return ingredientesOrder;
};

console.log(totlaIngredientsOrder());
